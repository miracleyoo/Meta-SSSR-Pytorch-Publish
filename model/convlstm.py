...
class ConvLSTM(nn.Module):
    ...
    def forward(self, input_tensor, hidden_state=None):
        ...
        for layer_idx in range(self.num_layers):
            ...
            last_state_list.append(h)  # 3*last h

        if not self.return_all_layers:
            layer_output_list = layer_output_list[-1]  # 1*all hs
            last_state_list = last_state_list[-1]  # 1 * last h
...
class BiConvLSTM(nn.Module):
    ...
    def forward(self, input_tensor, hidden_state=None):
        ...
        # Every h(cell's output) of the last layer.
        layer_output_list = []
        # Last (h, c): (cell's output, info high way) of the last layer.
        last_state_list = []
        seq_len = input_tensor.size(1)
        cur_layer_input = input_tensor
        ...
        for layer_idx in range(self.num_layers):
            ...
            output_inner_b = []
            for t in range(seq_len):
                hf, cf = self.cell_list[layer_idx](input_tensor=cur_layer_input[:, t, :, :, :],
                                                   cur_state=[hf, cf])

                hb, cb = self.cell_list[layer_idx](input_tensor=cur_layer_input[:, seq_len-t-1, :, :, :],
                                                   cur_state=[hb, cb])
                output_inner_f.append(hf)
                output_inner_b.append(hb)
            output_inner_b.reverse()
            output_f = torch.stack(output_inner_f,dim=1) # b, t, c, h, w
            output_b = torch.stack(output_inner_b,dim=1)
            output_fb = torch.cat([output_f,output_b],dim=2) # b, t, 2c, h, w
            output_fb = output_fb.view(B*seq_len,-1,H,W)
            layer_output = self.fuse_list[layer_idx](output_fb) # b, t, c, h, w
            layer_output = layer_output.view(B,seq_len,-1,H,W) # b, t, c, h, w
            cur_layer_input = layer_output

            layer_output_list.append(layer_output)  # 3*all hs
            last_state_list.append(layer_output[:,-1,:,:,:])  # 3*last [h]
